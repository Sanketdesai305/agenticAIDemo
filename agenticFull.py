...
def identify_project_type():
    ...
    'stack.yaml', 'CMakeLists.txt', 'Package.swift', 
    'pubspec.yaml', 'build.sbt', 'Makefile.PL', 'Build.PL',
    'build.gradle.kts'
]
...
- Include build, and artifact upload steps use all the latest stable versions.
...
for filename, content in project_files:
    ...
    if filename == "Cargo.toml" and re.search(r"\[package\]", content):
        print("🦀 Confirmed Rust Cargo project.")
    if filename == "Gemfile" and re.search(r"source\s+['\"]https://rubygems.org['\"]", content):
        print("💎 Confirmed Ruby project with Bundler.")
    if filename == "composer.json" and re.search(r'"require"\s*:', content):
        print("🐘 Confirmed PHP project with Composer.")
    if filename == "mix.exs" and re.search(r'defp\s+project', content):
        print("💧 Confirmed Elixir Mix project.")
    if filename == "CMakeLists.txt":
        print("🛠️ Confirmed CMake project.")
    if filename == "Package.swift":
        print("🍏 Confirmed Swift Package project.")
    if filename == "pubspec.yaml":
        print("🎯 Confirmed Dart/Flutter project.")
    if filename == "build.sbt":
        print("🛠️ Confirmed Scala SBT project.")
...
if project_type == "nodejs":
    ...
    elif any(file == "build.gradle" or file == "build.gradle.kts" for file, _ in project_files):
